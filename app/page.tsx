...
export default function CityPage() {
  ...
  return <main className={`city-app ${state.active ? 'is-playing' : ''} ${state.driving ? 'is-driving' : ''}`}>
    ...
    {state.ready && <>
      ...
      {!state.active && state.mode !== 'aerial' && <div className="entry-prompt"><Button className="enter-button" onClick={() => engine.current?.enter()}>{state.driving?<CarFront size={18}/>:<Footprints size={18} />}{state.driving?'继续驾驶':'进入街区'} <ArrowRight size={17} /></Button><span>{state.driving?'W 油门 · Shift 加速 · 空格+A/D 漂移':'WASD 移动 · F 上车 · J 出拳'}</span></div>}
      ...
      {state.driving && <div className="vehicle-status" role="status" aria-label={`驾驶 ${state.vehicleName}，时速 ${state.vehicleSpeed} 公里，${state.vehicleGear} 挡`}><div><CarFront size={14}/>{state.vehicleName}</div><p><b>{state.vehicleSpeed.toString().padStart(2,'0')}</b><span>km/h</span><strong>{state.vehicleGear}</strong></p><small>W 油门 · S 刹车 / 倒车</small><div className="driving-feedback">{state.vehicleDrift?'漂移中 · 松开空格恢复抓地':state.vehicleBoost?'加速中':'Shift 加速 · 空格 + A/D 漂移'}</div><div className="driving-buttons"><button onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);engine.current?.touchMove('ShiftLeft',true);}} onPointerUp={()=>engine.current?.touchMove('ShiftLeft',false)} onPointerCancel={()=>engine.current?.touchMove('ShiftLeft',false)}>Shift 加速</button><button onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);engine.current?.touchMove('Space',true);}} onPointerUp={()=>engine.current?.touchMove('Space',false)} onPointerCancel={()=>engine.current?.touchMove('Space',false)}>空格 漂移</button><button onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);engine.current?.touchMove('KeyX',true);}} onPointerUp={()=>engine.current?.touchMove('KeyX',false)} onPointerCancel={()=>engine.current?.touchMove('KeyX',false)}>X 急刹</button></div></div>}
      ...
      <aside className={`map-panel ${mapOpen ? '' : 'map-collapsed'}`}>
        ...
        {mapOpen && <><svg viewBox="0 0 300 300" className="minimap" role="img" aria-label="城市地图，青色箭头标注当前位置，金色车标为公交车">
